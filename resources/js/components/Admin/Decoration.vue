<template>
    <div class="row" v-if="props?.orderSummary?.timeSlot?.decorations">
        <div class="col-lg-2 mt-4 text-center" v-for="item in props.orderSummary.timeSlot.decorations" :key="item.id">
...
let state = decorations.value;
let existingDetails = props?.params?.existing?.decorations;

watch(state, () => {
    emit("updateState", {
        data: { decorations: state },
    });
});
