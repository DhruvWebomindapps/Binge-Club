        ...
        <div class="row mb-4">
            <div class="col col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ url('admin/master/cake/' . $cake->id) }}" enctype="multipart/form-data"
                            method="POST">
                            @csrf
                            {{ method_field('PUT') }}
                            <div class="row">
                                        ...
                                        <select class="form-select" aria-label="Default select example" name="city_id"
                                            ...
                                            @foreach ($cities as $city)
                                                <option value="{{ $city->id }}"
                                                    @if ($cake->city_id == $city->id) selected @endif>
                                                    {{ $city->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select class="form-select" aria-label="Default select example"
                                            ...
                                            @foreach ($locations as $location)
                                                <option value="{{ $location->id }}"
                                                    @if ($location->id == $cake->location_id) selected @endif>
                                                    {{ $location->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row mt-4">
                                    <div class="col-lg-6">
                                        ...
                                        <img src="{{ asset('storage/' . $cake->icon) }}" alt="" class="mt-3"
                                            style="width:100px; height:100px;">
                                    </div>
                                    <div class="col-lg-6">
                                        <label class="form-label">Priority</label>
                                        <input type="number" class="form-control" name="priority"
                                            value="{{ $cake->priority }}" required />
